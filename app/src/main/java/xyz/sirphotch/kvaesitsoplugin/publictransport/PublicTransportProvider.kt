...
class PublicTransportProvider : LocationProvider(
    ...
    override suspend fun get(
        ...
        val (prov, id) = id.split('/', limit = 2)
        val provider =
            prov.runCatching { Provider.valueOf(this) }.getOrNull() ?: return@withContext null
        val availableProviders = enabledProviders.firstOrNull() ?: return@withContext null
        if (!availableProviders.contains(provider)) return@withContext null

        with(NetworkProviderFactory.get(provider)) {
            if (!hasCapabilities(NetworkProvider.Capability.DEPARTURES)) return@with null

            // val reverseLoc = this.queryNearbyLocations(setOf(LocationType.STATION), PteLocation(LocationType.STATION, id), 0, 1)
            // println(reverseLoc.locations.firstOrNull()?.name)
            runCatching {
                queryDepartures(
                    id,
                    null,
                    7,
                    false
                ).stationDepartures.associate { it.location to it.departures }
                    .mapNotNull { it.toLauncherLocation(provider) }
            }.onFailure {
                Log.e("Sir-Photch", "${id().name} queryDepartures", it)
...
private fun Map.Entry<PteLocation, List<PteDeparture>>.toLauncherLocation(provider: Provider): Location? {
